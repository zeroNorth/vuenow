<template>
  <div class="div-info">
    <ul style="margin-top: 1.2rem;">
      <div class="type-selected">
        ...
        <div class="item-padding type-item"  @click="selectCard" v-else>
          ...
          <div class="fl">
            ...
            <p class="shadoww">单笔限额{{this.stokAmtLimitFomart}}元 单日限额{{this.dayAmtLimitFomart}}元</p>
          </div>
      ...
      <div class="top-tip">
        <div class="inputEdit">
          <div class="he unit-style">金额</div>
          <input type="text" placeholder="请输入充值金额" maxlength="11" class="he input-style" v-model="inputMoney" id="moneyInput" onkeyup="var p2 = parseFloat(value).toFixed(2);value = p2>=0?(/\.0?$/.test(value)?value:p2.replace(/0$/,'').replace(/\.0$/,'')):''" onblur="value = value.replace(/\.0*$/,'')"/>
        </div>
  ...
  export default {
    ...
    methods: {
      ...
      nextClick: function () {
        ...
        if (this.inputMoney.length > 0 && parseFloat(this.inputMoney) > 0) {
          if(this.listSelected&&parseFloat(this.inputMoney)>(parseFloat(this.listSelected.stok_amt_limit)/100)){
            Toast('交易金额不能大于单笔限额')
            return ;
          }
          //弹出密码框
